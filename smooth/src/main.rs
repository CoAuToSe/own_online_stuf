...
fn fs_main(in: VertexOutput) -> [[location(0)]] vec4<f32> {
    ...
    for (var i: u32 = 1u; i < 500u; i = i + 1u) {
        let rx: f32 = fract(f32(i)/500. * 1000.);
        let ry: f32 = fract(f32(i)/500. * 1001.);
        let sx: f32 = fract(f32(i)/500. * 1009.) - 0.5;
        let sy: f32 = fract(f32(i)/500. * 1011.);

        let xx = fract(rx + sx * uniforms.time / 4.) * 2.4 - 1.2;
        let yy = fract(ry + sy * uniforms.time / 4.) * 2.4 - 1.2;

        let r = (fract(f32(i)/500. * 1044.) + 1. / 2.) * 0.06 + 0.01;
