    ...
    fn update(&mut self, event: winit::event::WindowEvent) {
        match event {
            winit::event::WindowEvent::CursorMoved { position, .. } => {
                unsafe {
                    LOL = [
                        LOL[0] + (position.x - LOL2[0]),
                        LOL[1] + (position.y - LOL2[1]),
                    ];
                    LOL2 = [position.x, position.y];
                    println!("{:?}", LOL);
                }
                let norm_x = unsafe { LOL[0] } as f32 / self.camera.screen_size.0 as f32 - 0.5;
                let norm_y = unsafe { LOL[1] } as f32 / self.camera.screen_size.1 as f32 - 0.5;
                self.camera.angle_y = norm_x * 5.0;
                self.camera.angle_xz = norm_y;
            }
            winit::event::WindowEvent::Moved(lol) => {
                println!("{:?}", lol)
            }
            winit::event::WindowEvent::KeyboardInput { input, .. } => match input {
                winit::event::KeyboardInput {
                    ...
                    println!(
                        "{:?} {:?} {:?} ",
                        scancode, state, virtual_keycode
                    );
                    println!("{:?}", unsafe { LOL});
                    if let Some(idiot) = virtual_keycode {
                        match idiot {
                            winit::event::VirtualKeyCode::Left => unsafe { LOL[0] -= 5.0 },
                            winit::event::VirtualKeyCode::Right => unsafe { LOL[0] += 5.0 },
                            winit::event::VirtualKeyCode::Up => unsafe { LOL[1] += 5.0 },
                            winit::event::VirtualKeyCode::Down => unsafe { LOL[1] -= 5.0 },
                            _ => ()
                        }
                    };
...
static mut LOL: [f64; 2] = [0.0, 0.0];
static mut LOL2: [f64; 2] = [0.0, 0.0];
fn main() {
